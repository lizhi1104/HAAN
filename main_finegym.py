...
class training_set(Dataset):
    def __init__(self, features):
        vnames = np.load('annotations/FineGym/videoname.npy', allow_pickle=True)
        subset = np.load('annotations/FineGym/subset.npy', allow_pickle=True)
        labels_unique = np.load('annotations/FineGym/labels_unique.npy', allow_pickle=True)
        ...
        labels = np.load('annotations/FineGym/labels.npy', allow_pickle=True)
        labels = np.array([list(np.array(x)[:,1]) for x in labels])
        segments = np.load('annotations/FineGym/segments.npy', allow_pickle=True)
        ...
        for i in range(len(vnames)):
            ...
            self.features.append(self.pad(features[i]))
            ...
            self.truth_masks.append(self.get_truth_mask(labels[i], segments[i], [len(features[i]), NUM_FINE_CLASS]))
    ...
    def get_truth_mask(self, labels, segments, shape):
        assert len(labels) == len(segments)
        mask = torch.zeros(shape)
        for i in range(len(labels)):
            start, end = segments[i]
            start_clip = int(round(start * 24 / 16))
            end_clip = min([int(round(end * 24 / 16)) + 1, shape[0]])
...
class test_set(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return vname, feature, label, segments
...
def get_predictions_and_gts(predictions, segments, labels):
    templabelidx = list(range(99))
    predictions, c_score = get_predictions_mod_kmax(predictions, kmax_factor)
#     c_score = np.zeros((len(predictions), 99))
...
#             threshold = np.max(tmp) - (np.max(tmp) - np.min(tmp)) * 0.5
    ...
    return segment_predicts, segment_gts

def getLocMAP(segment_predicts, segment_gts, th):
    factor = 24.0 / 16.0
    ap = []
        ...
        for i in range(len(segment_predict)):
            ...
            for j in range(len(segment_gt)):
                if segment_predict[i][0] == segment_gt[j][0]:
                    gtstart = int(round(segment_gt[j][1]) * factor)
                    gtend = np.max([gtstart + 1, int(round(segment_gt[j][2] * factor))])
...
def getDetectionMAP(predictions, segments, labels):
    iou_list = [0.5]
    dmap_list = []
    print('Getting predictions')
    sys.stdout.flush()
    segment_predicts, segment_gts = get_predictions_and_gts(predictions, segments, labels)
    for iou in iou_list:
        print('Testing for IoU %f' %iou)
        sys.stdout.flush()
        dmap_list.append(getLocMAP(segment_predicts, copy.deepcopy(segment_gts), iou))
...
def generate_features(encoder):
    ...
    subset = np.load('annotations/FineGym/subset.npy', allow_pickle=True)
    features = []
    for vname, i3d_feature, sub in zip(vnames, i3d_features, subset):
        if sub != 'train':
            continue
...
def run_test(fine_encoder, fine_classifier, test_loader, exp_name, epoch):
    ...
    for batch in test_loader:
        vname, feature, label, segments = batch
    ...
    dmap, iou = getDetectionMAP(element_logits_stack, segments_stack, labels_stack)
...
def get_closest_vc_for_class(classifier_weights, vc, vc_loss_method, top_vc_method):
...
i3d_features = np.load('FineGym_i3d_fps24.npy', allow_pickle=True)
training_loader = DataLoader(training_set(i3d_features), batch_size=BATCH_SIZE, shuffle=True)
test_loader = DataLoader(test_set(i3d_features), batch_size=1, shuffle=False)
fine_to_coarse_mapping = json.load(open("annotations/FineGym/fine_to_coarse_mapping.json"))
...
logs_dir = "logs_finegym"
ckpt_dir = "ckpt_finegym"
os.makedirs(logs_dir, exist_ok=True)
os.makedirs(ckpt_dir, exist_ok=True)

base_runs = os.listdir("ckpt_finegym_fine_only_base_from_scratch")
base_runs = ["512_2048_0.5_kmax_2_0.0001",
             "1024_2048_0.5_kmax_2_0.0001",
             "512_2048_0.25_kmax_2_0.0001",
             "2048_2048_0.25_above_average_0.0001",
             "2048_2048_0.5_above_average_0.0001",
             "1024_2048_0.25_above_average_0.0001"]

hyperparameters = {
    "learning_rate": [0.0001, 0.00003, 0.00001],
    "num_fine_vc": [200, 300, 500],
    "clus_weight": [0.01, 0.001, 0.0001],
    "clustering_loss_weight": [True, False],
    "vc_weight": [0.01, 0.001, 0.0001],
    "vc_loss_method": ["mse", "cosine"],
    "top_vc_method": ["fixed_2", "fixed_3", "fixed_5", "dis_1.2", "dis_1.5", "dis_2"],
    "coarse_cls_weight": [0.1, 0.01, 0.001, 0.0001],
    "vc_fine_to_coarse_pool": ["mean", "max"],
}
...
for _ in range(20):
    ...
    torch.manual_seed(0)
    np.random.seed(0)
    fine_encoder = nn.DataParallel(Model(2048, feature_size, encoder_hidden_layer, dropout_rate)).to(DEVICE)
    fine_classifier = nn.DataParallel(Model(feature_size, NUM_FINE_CLASS, str(feature_size), dropout_rate)).to(DEVICE)
    fine_self_sup_model = nn.DataParallel(Model(feature_size, num_fine_vc, str(feature_size), dropout_rate)).to(DEVICE)
    coarse_classifier = nn.DataParallel(Model(feature_size, 1, str(feature_size), dropout_rate)).to(DEVICE)
    
    fine_encoder_pkl = f"ckpt_finegym_fine_only_base_from_scratch/{base_run}/fine_encoder_200.pkl"
    fine_classifier_pkl = f"ckpt_finegym_fine_only_base_from_scratch/{base_run}/fine_classifier_200.pkl"
    fine_encoder.load_state_dict(torch.load(fine_encoder_pkl))
    fine_classifier.load_state_dict(torch.load(fine_classifier_pkl))
    
    parameters = list(fine_encoder.parameters()) + list(fine_classifier.parameters()) + list(fine_self_sup_model.parameters())
    ...
    for e in range(epochs):
        ...
        if e % 5 == 0:
            ...
            generate_clustering_labels(fine_features, num_fine_vc, "fine", 0.1)
        ...
        for batch in training_loader:
            ...
            for j in range(len(seq_lens)):
                ...
                fine_closest_vcs = get_closest_vc_for_class(fine_classifier_weights, vc, vc_loss_method, top_vc_method)
                ...
                _, coarse_logit = coarse_classifier(coarse_vc)
                coarse_cls_loss += F.binary_cross_entropy_with_logits(coarse_logit.squeeze(1),
                                                                      coarse_labels[j].to(DEVICE),
                                                                      weight=coarse_weights[j].to(DEVICE))
                
            loss = fine_cls_loss + clus_weight * fine_clustering_loss + vc_weight * fine_vc_loss + coarse_cls_weight * coarse_cls_loss

            print(f'batch {count:04d} loss={loss.item():.5f} cls_loss={fine_cls_loss.item():.5f} clustering_loss={fine_clustering_loss.item():.5f} vc_loss={fine_vc_loss.item():.5f} coarse_cls_loss={coarse_cls_loss.item():.5f}')  
            sys.stdout.flush()
